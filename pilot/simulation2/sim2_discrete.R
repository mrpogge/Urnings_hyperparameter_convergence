...
#true values
pi_pl = rep(seq(0.1,0.9,0.1), each = 500)
...
jump = sample(100:400, 4500, replace = TRUE)
for(i in 1:4500){
...
for(pus in player_urn_sizes){ #4
    for(ads in 1:length(ad_text)){ #4
        ...
        r_pl = numeric(nplayers) + unlist(lapply(pi_pl[1:500], rbinom, n = 1, size = pus))
        ...
        results[counter:(counter+nplayers-1), 4] = jump
...
saveRDS(results, "sim2_discrete.rds")
