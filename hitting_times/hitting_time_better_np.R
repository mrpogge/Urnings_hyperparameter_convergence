      ...
      for(p in pu){ #2
        for(cs in player_percent){
          for(r in 1:nreps){
            ...
            r_pl = r_pl[indexes]
            pi_pl = pi_pl[indexes]
            Theta = matrix(rep(log(pi_pl / (1 - pi_pl)), times = ngames), ncol = ngames, nrow = nplayers, byrow = TRUE)
            Delta = matrix(rep(log(pi_it / (1 - pi_it)), times = ngames), ncol = ngames, nrow = nitems, byrow = TRUE)
...
saveRDS(results, "output/ht_better_np.rds")
