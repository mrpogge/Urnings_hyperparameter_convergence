...
for(i in 1:nrow(change_matrix_linear)){
  for(j in 1:ncol(change_matrix_linear)){
    baseline_matrix[i,j] = rbinom(1, size = as.numeric(post_hoc_symmetric[i, "player_urn_size"]), prob = change_matrix_linear[i,j])
    CI = binconf(baseline_matrix[i,j], as.numeric(post_hoc_symmetric[i, "player_urn_size"]), method = "wilson")[2:3]
...
saveRDS(baseline_linear, "post_hoc_symmetric_baseline.rds")