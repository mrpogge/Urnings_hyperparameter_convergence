...
# urn sizes
player_urn_sizes  =  c(8,16,32,64,80, 96, 112, 128)
...
# output
results = matrix(0, nrow = nplayers*8, ncol = 6 + ngames + ngames -1)
...
for(pus in player_urn_sizes){ #4
  for(ads in 1:length(ad_text)){ #4
    ...
    results[counter:(counter+nplayers-1), 4] = rep(NA, times = nplayers)
...
saveRDS(results, "post_hoc_moreiter.rds")
